...
VInv <- function(Z,ZId,D,sigma)
{
  ...
  if(det.check!=0){
    vInverse <- solve(sigma^2*ZId + quad.tform(D,Z),tol = 1e-90)
  }else{
...
VnotInv <- function(Z,ZId,D,sigma)
{
  v <- sigma^2*ZId + quad.tform(D,Z)
...
nlogdet <- function(LGroup) {
  nlogdetfun <- function(L) {
    if (!is.matrix(L)) {
      warning("Skipping non-matrix element in LGroup")
      return(0)
    }
    -1/2 * determinant(L, logarithm = TRUE)$modulus[1]
  }
  
  sum(sapply(LGroup, nlogdetfun))
}
...
objFunction <- function(xGroup,yGroup,zGroup,zIdGrp,b,L,sigma,lambda1,lambda2,nonpen.b, nonpen.L,penalty.b, penalty.L,ll1=ll1,ntot,ll2=NULL)
{
  ...
  pen.term.b=0
  if(penalty.b=="lasso"){
    pen.term.b=lambda1*sum(abs(b[-nonpen.b]))
  }else if(penalty.b=="scad"){
    pen.term.b = sum(scad_group(b[-nonpen.b], lambda1))
  }


  pen.term.L = 0
  if(penalty.L=="lasso"){
    pen.term.L = lambda2*L2norm
  }else if(penalty.L=="scad"){
    pen.term.L = sum(scad_group(L2norm, lambda2))
  }


  ll <- ll1 + ll2 + sum(mapply(tResLRes,LGroup,resGroup)) + pen.term.L + pen.term.b
...
ArmijoRule_b <- function(xGroup,yGroup,LGroup,b,j,cut,HkOldJ,HkJ,JinNonpen,lambda,nonpen,penalty,ll1,ll2,converged,control)
{
  ...
  fct <- objFunction_b(xGroup=xGroup,yGroup=yGroup,LGroup=LGroup,b=b,
  ...
  { 
    ...
    else deltak <- dk*grad + control$gamma*dk^2*HkJ + lambda*(abs(bJ+dk)-abs(bJ))
...
MyQuadFormInv <- function(M, x) {
  
  det.check = det(M)
  if(det.check>0){
    crossprod(x, solve(M, x, tol = 1e-90))
  }else{
    Minv <- ginv(M)
    Minvx <- Minv%*%x
    crossprod(x,Minvx)
...
armijoRule_L <- function(xGroup,yGroup,zGroup,L,l,k,grad,hessian,b,sigma,zIdGrp,linNonpen,lambda,nonpen,penalty,ll1,converged,control,ntot)
{
  L.new <- L
  Llk <- L[l,k]
  ...
  { 
    ...
    fctOld <- objFunction_L(xGroup=xGroup,yGroup=yGroup,zGroup=zGroup,zIdGrp=zIdGrp,b=b,L=L,sigma=sigma,lambda=lambda,nonpen=nonpen,penalty=penalty,ll1=ll1, ntot = ntot)
    
    for (j in 0:control$maxArmijo)
    { 
      L.new[l,k] <- Llk + control$a_init*control$delta^j*dk
      
      fctNew <- objFunction_L(xGroup=xGroup,yGroup=yGroup,zGroup=zGroup,zIdGrp=zIdGrp,b=b,L=L.new,sigma=sigma,lambda=lambda,nonpen=nonpen,penalty=penalty,ll1=ll1, ntot = ntot)
    ...
    fct = objFunction_L(xGroup=xGroup, yGroup=yGroup, zGroup=zGroup, zIdGrp=zIdGrp,
                        b=b, L=L, nonpen=nonpen, sigma=sigma, lambda=lambda, penalty=penalty, ll1=ll1, ntot=ntot)
  }
...
objFunction_L <- function(xGroup,yGroup,zGroup,zIdGrp,b,L,sigma,lambda,nonpen,penalty,ll1=ll1,ntot,ll2=NULL)
{
  ResAs <- function(x,y,b,activeSet) y-x[,activeSet,drop=FALSE]%*%b[activeSet,drop=FALSE]
  ...
  ll <- ll1/ntot + ll2/ntot + sum(mapply(tResLRes,LGroup,resGroup)) + pen.term.L
