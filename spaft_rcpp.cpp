...
double ObjFunction(Rcpp::List xGroup, Rcpp::List yGroup, Rcpp::List LGroup, arma::vec b_nonpen, arma::mat L_nonpen, double lambda1,double lambda2,
                   Rcpp::String penalty_b, Rcpp::String penalty_L, double ntot, double ll1){
  ...
  //double rt = rtvr+ll1+ll2+pen_b+pen_L;
  double rt = rtvr+ll1+ll2;
  double rtt = rt/ntot;
  return rtt+pen_b+pen_L;
}

// [[Rcpp::export]]
double DensityFunction(arma::mat x, arma::vec y, arma::mat V, arma::vec b, double ll1){
  
  //::uvec activeSet = find(b_nonpen!=0);
  //arma::vec b_active = b_nonpen(activeSet);
  ...
  double vi = as_scalar(vitemp*ri)*0.5;
  ...
  double abs_det_v = std::abs(det_v);
  double logdetv = log(abs_det_v);
  double ll2 = -logdetv*1/2;
  
  double ll1tmp = ll1*y.size();
  return rtvr-ll1tmp+ll2;
}

// [[Rcpp::export]]
double ObjFunction_L(Rcpp::List xGroup, Rcpp::List yGroup, Rcpp::List zGroup, Rcpp::List zIdGrp,
                     arma::vec b_nonpen, arma::mat L, arma::uvec nonpen, double sigma, double lambda, Rcpp::String penalty, double ntot, double ll1){
...
Rcpp::List ArmijoRule_L(Rcpp::List xGroup, Rcpp::List yGroup, Rcpp::List zGroup, arma::mat L, int l, int k, double grad, double hessian, 
                        ...
                        double ll1, double gamma, int maxArmijo, double a_init, double delta, double rho, int converged, double ntot){
  //grad = grad/ntot;
  ...
  double fct = ObjFunction_L(xGroup=xGroup, yGroup=yGroup, zGroup=zGroup, zIdGrp=zIdGrp,
                             b=b, L=L, nonpen=nonpen, sigma=sigma, lambda=lambda, penalty=penalty, ntot=ntot, ll1=ll1);
  ...
  if(dk!=0){
    ...
    for(int j=0; j<=maxArmijo; j++){
      ...
      double fctNew = ObjFunction_L(xGroup=xGroup, yGroup=yGroup, zGroup=zGroup, zIdGrp=zIdGrp,
                                    b=b, L=L_new, nonpen=nonpen, sigma=sigma, lambda=lambda, penalty=penalty, ntot=ntot,ll1=ll1);
